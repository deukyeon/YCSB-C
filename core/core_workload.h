...
public:
   ...
   ///
   virtual void
   InitLoadWorkload(const utils::Properties &p,
   ...
   virtual void
   InitPairs(std::vector<ycsbc::DB::KVPair> &values);
   virtual void
   BuildValues(std::vector<ycsbc::DB::KVPair> &values);
   virtual void
   UpdateValues(std::vector<ycsbc::DB::KVPair> &values);
   virtual void
   BuildUpdate(std::vector<ycsbc::DB::KVPair> &update);
...
protected:
   ...
   std::string
   BuildKeyName(uint64_t key_num);
   void
   UpdateKeyName(uint64_t key_num, std::string &buffer);
...
inline void
CoreWorkload::InitKeyBuffer(std::string &buffer)
{
   buffer = BuildKeyName(0);
}

inline void
CoreWorkload::NextSequenceKey(std::string &buffer)
{
...
inline void
CoreWorkload::UpdateKeyName(uint64_t key_num, std::string &buffer)
{
   if (!ordered_inserts_) {
      key_num = utils::Hash(key_num);
   }
